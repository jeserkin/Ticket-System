...
if(@$_REQUEST['ticket'] == "") {
	...
	if(isset($_POST['tAnswer'])) $ticket->replyToTicket($_REQUEST['ticket'], $_SESSION['id'], $_POST['answerCont']);
...
?>
				...
				<?php
					if($ticket->checkForReplies($_REQUEST['ticket'])) {
						$result = $ticket->getTicketReplies($_REQUEST['ticket']);
						while($treplies = $db->fetchAssoc($result)) {
				...
				?>
				<?php if($ticket->checkTicketStatus($_REQUEST['ticket'])) { ?>
				<div id="tClose">
				...
				<div class="ticketReply">
					<div class="innerReply">
						<form action="" method="post">
							...
							<div class="whoEmail">
								<label for="whoMail">Email: </label>
								<span id="whoMail"><?php echo $_SESSION['email']; ?></span>
							</div>
							<div class="answer">
								<textarea name="answerCont" rows="12" cols=""></textarea>
							</div>
